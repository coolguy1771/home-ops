...
- Monitoring: k8s-monitoring Alloy scrapes ServiceMonitors/PodMonitors and ships metrics via OTLP to external Mimir (`mimir.cloud.witl.xyz`) and logs to Loki (`loki.cloud.witl.xyz`); disable Rook `PrometheusJobMissing`/`PrometheusJobExporterMissing` rules (synthetic `up{job=...}` unavailable in OTLP pipeline)
