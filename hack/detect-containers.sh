...
# fail
images+=("$(yq eval '[.. | select(has("repository")) | select(has("tag"))] | .[] | .repository + ":" + .tag' ./cluster/apps/networking/ddns/cloudflare-ddns/cron-job.yaml)")
images+=("$(yq eval '[.. | select(has("repository")) | select(has("tag"))] | .[] | .repository + ":" + .tag' ./cluster/apps/networking/ddns/cloudflare-ddns/cron-job.yaml)")
...
# pass
images+=("$(yq eval '.spec.jobTemplate.spec.template.spec.containers.[].image' ./cluster/apps/networking/ddns/cloudflare-ddns/cron-job.yaml)")
...
# fail
images+=("$(yq eval '.services.*.image' ./cluster/apps/networking/ddns/cloudflare-ddns/cron-job.yaml)")
